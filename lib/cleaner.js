...
export default async function runCleaner({
  ...
  try {
    ...
    console.log(
      chalk.green(
        `✔ Branches eligible: ${staleBranches.length}`
      )
    );
    console.log(
      chalk.green(
    ...
    if (failedCount > 0) {
      console.log(
        chalk.red(`✖ Failed deletions: ${failedCount}`)
      );
    }
    ...
    console.log(
      chalk.gray(`🚯 Excluded branches: ${excludedSummary}`)
    );

    if (failedCount > 0) {
      // Runtime failure (partial success)
      process.exit(2);
    }
